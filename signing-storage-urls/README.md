...
However on App Engine the [`blob.generate_signed_url(..)` method](https://docs.cloud.google.com/python/docs/reference/storage/latest/summary_method#google_cloud_storage_blob_Blob_generate_signed_url_summary) does not work with the default service account credentials [returned by `google.auth.default()`](https://google-auth.readthedocs.io/en/latest/reference/google.auth.html#google.auth.default).

There's a [Python code sample](https://docs.cloud.google.com/storage/docs/samples/storage-generate-signed-url-v4#code-sample), but using that as-is will cause an exception when deployed to App Engine standard. The exception reads:

> _AttributeError: you need a private key to sign credentials.the credentials you are currently using <class 'google.auth.compute_engine.credentials.Credentials'> just contains a token. see https://googleapis.dev/python/google-api-core/latest/auth.html#setting-up-a-service-account for more details._

One could also export the service account key to a JSON credentials file, then create credentials from that file, but this approach has [disadvantages](https://medium.com/@jryancanty/stop-downloading-google-cloud-service-account-keys-1811d44a97d9).
...
- In your Python code, obtain the service account's email address and an access token (see code in `main.py`). Call the blob signing API with the email address and token.
